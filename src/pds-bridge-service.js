...
module.exports = {
	...
	queryNhsNumber: function(req, res) {
		try {
            
			var correlationId = commonTools.generateGuid();
			var nhsNumber = req.query.nhsNumber;
			
			commonTools.consoleDumpText('debug', 'nhsNumber' , nhsNumber);
			
			var queueItem = {
				'correlationId': correlationId,
				'nhsNumber' :  encodeURIComponent(nhsNumber),
				'request': req,
				'response': res,
				'sent': false
				};
	...
	enqueueFromWorkerBackToBridge: function(req, res) {
		...
		queueItem.response.send(parsedResponse.pdsData); //back to requester
